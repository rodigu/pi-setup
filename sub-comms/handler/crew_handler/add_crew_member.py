...
from crew_handler.interfaces.crew_meta import CrewID, read_crew_id, get_crew_path, get_member_path

_VERBOSE=1

def _spawn_pi_instance(member_id: MemberID, member_parameters: CrewMemberParameters) -> MemberPID:
    """Spawns pi instance subprocess.
...
def _update_crew_json():
    """Updates the `members.json` file for each member.
    """

def _parse_member_prompt(crew_id: CrewID, member_id: MemberID) -> str:
...
def _get_member_id(skill:str) -> MemberID:
    ...
    Examples:
        >>> _get_member_id(skill="revise")
        'revise-0173'
        >>> _get_member_id(skill="code-review")
        'code-review-9812'
    """
...
if __name__=="__main__":
    ...
    parser.add_argument("filepath", help="New crew member JSON filepath.", type=str, required=True)
    parser.add_argument("-v","--verbose", help="Verbose mode.", type=bool, default=1)

    if not _is_crew_running():
        raise RuntimeError("No active crew handler found. Start a crew handler to add members.")
